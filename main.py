...
class Box(object):
    ...
    def glow(self, endcolor= [37,115,193]):
        self.endcolor = endcolor
        #print(self.boxColour, end=" ")
        #print(self.endcolor)
        
        rate = 1

        for i in range(3):
            if not self.cycle:                  # first cycle
                if self.boxColour[i] >= self.endcolor[i] + rate:
                    self.boxColour[i] -= rate
                
                if self.boxColour[i] <= self.endcolor[i] - rate:
                    self.boxColour[i] += rate
                
                if self.boxColour == self.endcolor:
                    self.cycle = True
            else:                               # second cycle
                if self.boxColour[i] >= self.defaultColor[i] + rate:
                    self.boxColour[i] -= rate
                
                if self.boxColour[i] <= self.defaultColor[i] - rate:
                    self.boxColour[i] += rate
                
                if self.boxColour == self.defaultColor:
                    self.cycle = False
                    self.endcolor = [37,115,193]
                    self.donecycle = True
...
def newSequence(): # Controls the sequence of flashing boxes
    ...
    for i in sequence:
        enumerate = sequence.index(i)
        print(sequence)
        if enumerate == 0:
                boxMatrix[i[0]][i[1]].glow([255,255,255]) # makes the random box in the sequence glow 
        elif boxMatrix[sequence[enumerate-1][0]][sequence[enumerate-1][1]].donecycle == True :
                boxMatrix[i[0]][i[1]].glow([255,255,255]) # makes the random box in the sequence glow 
highScore = HighScore()
highScore.readScore()
score = 0
class Screen:
    ...
    def main(self):
        while self.play == 1: # While loop for the main game
                        ...
                        for j in i:
                            if j.mouseDetection(pygame.mouse.get_pos()) == True: # Checks if the mouse is in a box
                                if j.x // 153 == self.sequence[self.correctClicks][1] and j.y // 153 == self.sequence[self.correctClicks][0]: # checks if the player has clicked on the correct box in the sequence
                                    ...
                                    j.glow([0,255,0]) # makes the box glow white if the player has clicked the correct box
                                    ...
                                    j.glow([255,0,0]) # Makes the box glow red if the player has clicked the wrong box
            ...
            if self.correctClicks == len(self.sequence): # checks if the player has completed the round
            ...
            for i in range(3):
                for j in range(3):
                    boxMatrix[i][j].glow(boxMatrix[i][j].endcolor)
            pygame.display.update() # Updates the screen
    def death(self):
